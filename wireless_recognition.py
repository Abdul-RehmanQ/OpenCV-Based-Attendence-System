...
def get_known_faces_from_db():
    ...
    known_names = []
    known_encodings = []
    unique_students = set()  # to count distinct students
    try:
...
def handle_add_new_face():
    ...
    while adding_more_faces:
        ...
        # Ask if the user wants to add another face for the same student
...
while True:
    ...
    if choice == "1":
        if handle_add_new_face():
            # Reload faces if new students were added successfully
            known_names, known_face_encodings = get_known_faces_from_db()
    elif choice == "2":
...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(
        ...
        if face_distances[best_match_index] < RECOGNITION_THRESHOLD:
            ...
            color = (0, 0, 255)  # Red for unknown student
